...
contract ThamesBoats is ReentrancyGuard {
  ...
  // struct of boats details, clubs, prices, stock
  struct Boat {
      string name;
      string description;
      uint256 price; 
  ...
  event LogBoatStatusToggled(uint256 indexed clubId, uint256 indexed boatId, uint256 boatStatus);
  ...
  modifier ownsThisClub(uint256 _clubId) {
      require((clubs[_clubId].owner == msg.sender), 
              "caller is not owner of the specified Club nor a Market Admin");
      _;
  }

  /// modifier, checks that caller owns the given boat
  modifier ownsThisBoat(uint256 _boatId) {
      uint256 ownsClubId = ownertoClub[msg.sender];
      require(boatIdtoClubId[_boatId] != 0, "this boat is not owned by any club - does not exist");
      require(ownsClubId == boatIdtoClubId[_boatId], "you are not the owner of this boat");
      _;
  }


  /// @notice constructor funtion
  /// @dev assign the owner the market admin role
  constructor() public {
      owner = msg.sender;
  ...
  /// @param _clubId of the club details requested
  ...
  function getClub(uint256 _clubId) 
      external
      view 
      returns(
          address,
  ...
  /// @param _boatId for required boat
  ...
  function getBoat(uint256 _boatId) 
      external 
      view
      returns(
          string memory,
  ...
  function getBoatsIdbyClubId(uint256 _clubId) external view returns(uint256[] memory) {
      return clubBoats[_clubId];      
  }

  /// @notice Add boat details, mapped to the callers club ID
  /// @dev The caller must have the club owner role and have a club
  /// @param _name, name or title of the boat
  /// @param _description, details about the boat for sale
  /// @param _price, price in Wei
  /// @param _quantity, quantity of the boat
      ...
      returns(uint256)
  {
      ...
      emit LogBoatAdded(myClubId, boatIdSeq, _price, _quantity, msg.sender);
      return boatIdSeq;
  }   
 
  /// @notice Remove a boat by given boat ID
  /// @dev The caller must be the owner of the given boat ID
  /// @param _boatId, ID of the boat to be marked as Removed
  /// @return bool
  function removeBoat(uint256 _boatId)
      external
      ownsThisBoat(_boatId)
      ifMarketNotSuspended
      returns(bool)
  { 
      // Check that mapping exist by ensuring nane length is non-zero
      uint256 nameLength = bytes(boats[_boatId].name).length;
      if(nameLength > 0 &&
         boats[_boatId].boatStatus != BoatStatus.Removed)
      {
          boats[_boatId].boatStatus = BoatStatus.Removed;  
          emit LogBoatRemoved(_boatId, msg.sender);
      }
      return true;
  }
  ...
  /// @dev The caller must be the owner of the given boat ID
  /// @param _boatId of boat to change status
  /// @return bool
  function toggleBoatStatus(uint256 _boatId)
      external
      ownsThisBoat(_boatId)
      ifMarketNotSuspended
      returns(bool)
  {
      if (boats[_boatId].boatStatus == BoatStatus.ForSale) {  
          (boats[_boatId].boatStatus = BoatStatus.NotForSale);
      } else {
          (boats[_boatId].boatStatus = BoatStatus.ForSale);
      } 
      uint256 boatStatus = uint256(boats[_boatId].boatStatus);
      emit LogBoatStatusToggled(clubId, boatId, boatStatus);
      return true;
  }
  ...
  /// @param _boatId, ID of boat to purchase
  /// @param _quantity, number of the boat to purcahse
  /// @return purchaseId, the ID of the purchase recorded in the purcahses mapping
  function purchaseBoat(uint256 _boatId, uint256 _quantity)
      external
      payable 
      ifMarketNotSuspended
      nonReentrant
      returns(uint256)
  {
      ...
      require(boats[_boatId].boatStatus == BoatStatus.ForSale, "boat not for sale");
      require(boats[_boatId].quantity >= _quantity, "not enough boats available");
      uint256 costOfSale = SafeMath.mul(boats[_boatId].price, _quantity);
      require(msg.value >= costOfSale, "not enough funds sent for purchase");
      uint256 clubIdOfBoat = boatIdtoClubId[_boatId];
      ...
      boats[_boatId].quantity = SafeMath.sub(boats[_boatId].quantity, _quantity);
  ...
  /// @param _name name of the rowing boat club
  /// @param _location location of the club
  function addClub(string calldata _name, string calldata _location) 
      external 
      hasClubOwnerRole 
      ifMarketNotSuspended 
  {
  ...
  /// @param _clubId ID of the club to be removed
  function removeClub(uint256 _clubId) 
      external 
      ownsThisClub(_clubId)
      ifMarketNotSuspended 
  {
      if(clubs[_clubId].clubStatus != ClubStatus.Removed) {
          clubs[_clubId].clubStatus = ClubStatus.Removed;
          emit LogClubRemoved(_clubId, msg.sender);
      }
  }

  /// @notice Mark the given club as Closed
  /// @dev May only be called by the owner of the given club
  /// @param _clubId ID of the club to be closed
  ...
  {   
      ...
      if(clubs[_clubId].clubStatus == ClubStatus.Open) {
          clubs[_clubId].clubStatus = ClubStatus.Closed;
          emit LogClubClosed(_clubId, msg.sender);
      }   
  }
